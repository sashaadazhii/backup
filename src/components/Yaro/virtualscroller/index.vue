      ...
      <div :class="loaderClass" v-if="!loaderDisabled && showLoader && d_loading">
        <template v-if="$slots && $slots.loader">
          <template v-for="(_, index) of loaderArr" :key="index">
            <slot
              name="loader"
              :options="getLoaderOptions(index, isBoth() && {numCols: d_numItemsInViewport.cols})"
            ></slot>
          </template>
        </template>
  ...
  methods: {
    ...
    scrollToIndex(index, behavior = 'auto') {
      ...
      if (both) {
        ...
        if (newFirst !== first) {
          horizontal
            ? scrollTo(calculateCoord(newFirst, itemSize, contentPos.left), 0)
            : scrollTo(0, calculateCoord(newFirst, itemSize, contentPos.top))
          this.first = newFirst
        }
      }
    },
    ...
    calculateNumItems() {
      ...
      const calculateNumItemsInViewport = (_contentSize, _itemSize) =>
    ...
    calculateOptions() {
      ...
      const calculateLast = (_first, _num, _numT, _isCols) =>
        this.getLast(_first + _num + (_first < _numT ? 2 : 3) * _numT, _isCols)
      const last = both
        ? {
    ...
    setContentPosition(pos) {
      if (this.content) {
        ...
        if (both) {
          setTransform(
            calculateTranslateVal(first.cols, this.itemSize[1]),
            calculateTranslateVal(first.rows, this.itemSize[0])
          )
        } else {
    ...
    onScrollPositionChange(event) {
      ...
      if (both) {
        ...
        isRangeChanged =
          (newFirst.rows !== this.first.rows && newLast.rows !== this.last.rows) ||
          (newFirst.cols !== this.first.cols && newLast.cols !== this.last.cols)
      ...
      if (items && !this.d_loading) {
        if (this.isBoth()) {
          return items
            ...
            .map(item => (this.columns ? item : item.slice(this.first.cols, this.last.cols)))
