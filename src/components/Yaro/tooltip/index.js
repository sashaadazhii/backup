import {DomHandler, ZIndexUtils, UniqueComponentId, ConnectedOverlayScrollHandler} from '../utils'
...
function create(el) {
  ...
  tooltipArrow.className = 'y-tooltip-arrow'
  container.appendChild(tooltipArrow)

  let tooltipText = document.createElement('div')
  tooltipText.className = 'y-tooltip-text'
...
function preAlign(el, position) {
  ...
  tooltipElement.className = `y-tooltip y-tooltip-${position} ${el.$_ptooltipClass || ''}`
}
...
const Tooltip = {
  beforeMount(el, options) {
    ...
    target.$_ptooltipZIndex = options.instance.$yaro && options.instance.$yaro.config && options.instance.$yaro.config.zIndex.tooltip
    bindEvents(target)
  },
  unmounted(el) {
    let target = getTarget(el)
