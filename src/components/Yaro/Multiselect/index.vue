<template>
  <div ref="container" :class="containerClass" @click="onClick">
    ...
    <div v-else class="y-multiselect-inner">
      ...
      <div class="y-multiselect-trigger" role="button" aria-haspopup="listbox" :aria-expanded="overlayVisible">
          ...
          <div class="y-multiselect-items-wrapper" :style="{'max-height': virtualScrollerDisabled ? scrollHeight : ''}">
            ...
            >
              <template v-slot:content="{styleClass, contentRef, items, getItemOptions, contentStyle}">
                <ul :ref="contentRef" :class="['y-multiselect-items', styleClass]" :style="contentStyle" role="listbox" aria-multiselectable="true">
                  <template v-if="!optionGroupLabel">
                    <li
                      v-for="(option, i) of items"
                      :class="[
                        'y-multiselect-item',
                        {'y-multiselect-item--highlight': isSelected(option), 'y-multiselect-item--disabled': isOptionDisabled(option)}
                      ]"
                      role="option"
                  ...
                  <template v-else>
                    <template v-for="(optionGroup, i) of items" :key="getOptionGroupRenderKey(optionGroup)">
                      ...
                      <li
                        ...
                        :class="['p-multiselect-item', {'p-highlight': isSelected(option), 'p-disabled': isOptionDisabled(option)}]"
                        role="option"
                        :aria-selected="isSelected(option)"
                        :key="getOptionRenderKey(option)"
                        @click="onOptionSelect($event, option)"
  ...
  computed: {
    ...
    containerClass() {
      return [
        'y-multiselect',
        {
          ...
          'y-multiselect--disabled': this.disabled,
          'y-multiselect--focus': this.focused
    ...
    labelClass() {
      return [
        'y-multiselect-label',
        {
          'y-multiselect-placeholder': this.label === this.placeholder,
          'y-multiselect-label-empty': !this.placeholder && (!this.modelValue || this.modelValue.length === 0)
        }
      ]
    },
    panelStyleClass() {
      return ['y-multiselect-panel', this.panelClass]
    },
    label() {
      let label
      ...
      if (this.selectAll !== null) {
        ...
        if (this.filterValue && this.filterValue.trim().length > 0) {
          ...
          if (this.modelValue && this.options) {
            let optionCount = 0
            if (this.optionGroupLabel) this.options.forEach(optionGroup => (optionCount += this.getOptionGroupChildren(optionGroup).length))
