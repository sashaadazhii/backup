<template>
  <div ref="container" :class="containerClass" :style="style">
    <div v-if="!inline" class="y-field__wrapper">
      <label class="y-field__label">
        <i :class="['y-field__icon', icon]" />
        <input
          :ref="inputRef"
          v-if="!inline"
          type="text"
          :class="['y-field__input', inputClass]"
          :style="inputStyle"
          @input="onInput"
          v-bind="$attrs"
          @focus="onFocus"
      ...
      >
        ...
        >
          <template v-if="!timeOnly">
            <div class="y-datepicker__group-container">
              <div class="y-datepicker__group" v-for="(month, groupIndex) of months" :key="month.month + month.year">
                <div class="y-datepicker__header">
                  <slot name="header"></slot>
                  <button
                    class="y-datepicker__prev"
                    v-show="groupIndex === 0"
                    ...
                    :disabled="$attrs.disabled"
                  >
                    <i class="y-datepicker__prev-icon i-arrow_back_ios1" />
                  </button>
                  <div class="y-datepicker__title">
                    <button
                      type="button"
                      @click="switchToMonthView"
                      @keydown="onContainerButtonKeydown"
                      v-if="currentView === 'date'"
                      class="y-datepicker__title-month"
                    ...
                    <button
                      ...
                      class="y-datepicker__title-year"
                      :disabled="switchViewButtonDisabled"
                    >
                      {{ getYear(month) }}
                    </button>
                  ...
                  <button
                    ...
                    :disabled="$attrs.disabled"
                  >
                    <i class="y-datepicker__next-icon i-arrow_forward_ios1" />
                  </button>
                </div>
                <div class="y-datepicker__calendar-container" v-if="currentView === 'date'">
                  <table class="y-datepicker__calendar">
                    <thead>
                      <tr>
                        <th scope="col" v-if="showWeek" class="y-datepicker__weekheader y-datepicker--disabled">
                    ...
                    <tbody>
                      <tr v-for="(week, i) of month.dates" :key="week[0].day + '' + week[0].month">
                        <td v-if="showWeek" class="y-datepicker__weeknumber">
                          <span class="y-datepicker--disabled">
                        ...
                        <td
                          ...
                          :class="{'p-datepicker-other-month': date.otherMonth, 'y-datepicker__today': date.today}"
                        >
                          <span
...
<script>
/* eslint-disable */
import {ConnectedOverlayScrollHandler, DomHandler, ZIndexUtils, UniqueComponentId} from '../utils'
...
export default {
  ...
  methods: {
    ...
    onOverlayEnter(el) {
      ...
      if (this.autoZIndex) {
        if (this.touchUI) ZIndexUtils.set('modal', el, this.baseZIndex || this.$yaro.config.zIndex.modal)
        else ZIndexUtils.set('overlay', el, this.baseZIndex || this.$yaro.config.zIndex.overlay)
      }
    ...
    isNavIconClicked(event) {
      return (
        DomHandler.hasClass(event.target, 'y-datepicker__prev') ||
        DomHandler.hasClass(event.target, 'y-datepicker__prev-icon') ||
        DomHandler.hasClass(event.target, 'y-datepicker__next') ||
        DomHandler.hasClass(event.target, 'y-datepicker__next-icon')
      )
    },
    alignOverlay() {
      if (this.touchUI) {
          ...
          if (literal) {
            ...
            switch (format.charAt(iFormat)) {
              case 'd':
                ...
                break
              case 'D':
                output += formatName('D', date.getDay(), this.$yaro.config.locale.dayNamesShort, this.$yaro.config.locale.dayNames)
                break
              case 'o':
                output += formatNumber(
                  'o',
              ...
              case 'm':
                ...
                break
              case 'M':
                output += formatName('M', date.getMonth(), this.$yaro.config.locale.monthNamesShort, this.$yaro.config.locale.monthNames)
                break
              case 'y':
    ...
    parseDate(value, format) {
      ...
      let iFormat,
        ...
        iValue = 0,
        shortYearCutoff =
          typeof this.shortYearCutoff !== 'string' ? this.shortYearCutoff : (new Date().getFullYear() % 100) + parseInt(this.shortYearCutoff, 10),
        year = -1,
        month = -1,
        day = -1,
        doy = -1,
        ...
        if (literal) {
          ...
          switch (format.charAt(iFormat)) {
            case 'd':
              ...
              break
            case 'D':
              getName('D', this.$yaro.config.locale.dayNamesShort, this.$yaro.config.locale.dayNames)
              break
            case 'o':
              doy = getNumber('o')
              break
            case 'm':
              ...
              break
            case 'M':
              month = getName('M', this.$yaro.config.locale.monthNamesShort, this.$yaro.config.locale.monthNames)
              break
            case 'y':
              year = getNumber('y')
              break
    ...
    updateFocus() {
      ...
      if (this.navigationState) {
        if (this.navigationState.button) {
          ...
          if (this.navigationState.backward) DomHandler.findSingle(this.overlay, '.y-datepicker__prev').focus()
          else DomHandler.findSingle(this.overlay, '.y-datepicker__next').focus()
        } else {
    ...
    getMonthName(index) {
      return this.$yaro.config.locale.monthNames[index]
    },
    getYear(month) {
      ...
      if (this.numberOfMonths > 1 && this.responsiveOptions) {
        ...
        if (this.responsiveOptions) {
          ...
          for (let i = 0; i < responsiveOptions.length; i++) {
            ...
            let styles = `
                            .p-datepicker[${this.attributeSelector}] .p-datepicker-group:nth-child(${numMonths}) .y-datepicker__next {
  ...
  computed: {
    ...
    containerClass() {
      return [
        ...
        this.class,
        {
          'y-calendar--timeonly': this.timeOnly,
          'y-calendar--disabled': this.$attrs.disabled,
          'y-calendar--filled': this.modelValue,
          'y-calendar--focus': this.focused
          // TODO: calendar or input
          // 'y-inputwrapper-filled': this.modelValue,
          // 'y-inputwrapper-focus': this.focused
        }
      ]
    },
    panelStyleClass() {
      return [
        ...
        this.panelClass,
        {
          'y-datepicker--inline': this.inline,
          ...
          'p-input-filled': this.$yaro.config.inputStyle === 'filled',
          'p-ripple-disabled': this.$yaro.config.ripple === false
        }
      ]
    },
    months() {
    ...
    weekDays() {
      let weekDays = []
      let dayIndex = this.$yaro.config.locale.firstDayOfWeek
      for (let i = 0; i < 7; i++) {
        weekDays.push(this.$yaro.config.locale.dayNamesMin[dayIndex])
    ...
    sundayIndex() {
      return this.$yaro.config.locale.firstDayOfWeek > 0 ? 7 - this.$yaro.config.locale.firstDayOfWeek : 0
    },
    datePattern() {
      return this.dateFormat || this.$yaro.config.locale.dateFormat
    },
    yearOptions() {
      if (this.yearRange) {
        let $vm = this
    ...
    monthPickerValues() {
      ...
      for (let i = 0; i <= 11; i++) {
        monthPickerValues.push(this.$yaro.config.locale.monthNamesShort[i])
      }
    ...
    todayLabel() {
      return this.$yaro.config.locale.today
    },
    clearLabel() {
      return this.$yaro.config.locale.clear
    },
    weekHeaderLabel() {
      return this.$yaro.config.locale.weekHeader
    },
    monthNames() {
      return this.$yaro.config.locale.monthNames
    },
    appendDisabled() {
