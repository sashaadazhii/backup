<template>
    <div class="header__wrapper">
        <div class="header__title">
            ...
            <Label v-if="order.vehicleInShop" class="header__label" icon="i-directions_car" border iconSize="18px"
                   size="large"/>
            <Label v-if="order.talkSA" class="header__label" icon="i-help" border iconColor="#FF9B70" iconSize="18px"
                   size="large"/>
            <Label v-if="order.partsOrdered" class="header__label" icon="i-build" border iconColor="#6B7280"
                   iconSize="18px" size="large"/>
        </div>
        <div class="header__nav">
            <router-link v-if="uid === 'new'" class="header__nav-link" :to="`/work-order/${uid}/general`">General
            </router-link>
        ...
        <div v-else class="header__menu">
            ...
            <div v-if="!isStart" class="header__timer" :class="{'-start': isStart}" @click="start">
                <div v-if="!isStart" class="header__timer-start"><i class="i-play_circle_filled"/> <span>Start Work Order</span>
                </div>
            </div>
    ...
    methods: {
        ...
        saveNewOrder() {
            const formattedOrderToSave = {
                ...
                customerRequests: this.order?.customRequests || [],
                scheduling: {
                    allTime: `${this.order?.customRequests?.reduce((n, {time}) => n + time, 0)} h` || ''
                },
